...
/// Trait to read account balances and nonces from the underlying state or a fallback.
pub trait LatticeStateProvider: Send + Sync + std::fmt::Debug + 'static {
    ...
    /// Retrieve the state nonce of the given address.
    fn get_nonce(&self, address: alloy_primitives::Address) -> u64;
}
...
impl LatticeStateProvider for () {
    ...
    fn get_nonce(&self, _address: alloy_primitives::Address) -> u64 {
        0
    }
}
...
pub fn compute_lattice_balance(
    ...
    for (hash, block) in &reg.lattice_blocks {
        // If we are the sender of a lattice Send block: deduct the sent amount from our lattice balance
        ...
        // If we are the recipient of a lattice Send block: add it if claimed or pending EVM transfer
        if let crate::stateless::LatticePayload::Send { recipient, amount } = &block.payload {
            if *recipient == address {
                if claimed.contains(hash) || initial_balance == alloy_primitives::U256::ZERO {
    ...
    B: reth_primitives_traits::Block,
{
    ...
    async fn validate_transaction(
        ...
        tracing::info!(?sender, hash = ?transaction.hash(), "ENTERING SovereignQuantumTransactionValidator::validate_transaction");
        ...
        let is_sys_target = if let Some(to) = transaction.to() {
            crate::system_registry::is_system_address(&to)
        } else {
            false
        };
        if is_did_reg {
            if let Ok(reg) = registry_lock.read() {
                if reg.address_to_did.contains_key(&sender) {
                    tracing::warn!(?sender, "Rejecting DID registration in pool: Identity already registered (replay prevention)");
                    return TransactionValidationOutcome::Invalid(
                        transaction,
                        InvalidTransactionError::TxTypeNotSupported.into(),
                    );
                ...
                if let Some(did) = reg.get_did_by_address(&sender) {
                    if reg.is_did_fully_registered(&did) || reg.is_did_registered(&did) || reg.identities.contains_key(&did) {
                        tracing::warn!(?sender, ?did, "Rejecting DID registration in pool: Identity already registered (replay prevention)");
                        return TransactionValidationOutcome::Invalid(
                            transaction,
                            InvalidTransactionError::TxTypeNotSupported.into(),
                        );
                ...
                let did_str = format!("did:sovereign:{}:{}", reg.chain_id, sender.to_string().to_lowercase());
                let did_str_hex = format!("did:sovereign:{}:{:#x}", reg.chain_id, sender);
                let did_str_no_prefix = format!("did:sovereign:{}:{:x}", reg.chain_id, sender);
                if reg.identities.contains_key(&did_str) || reg.peer_keys.contains_key(&did_str)
                    || reg.identities.contains_key(&did_str_hex) || reg.peer_keys.contains_key(&did_str_hex)
                    || reg.identities.contains_key(&did_str_no_prefix) || reg.peer_keys.contains_key(&did_str_no_prefix) {
                    tracing::warn!(?sender, ?did_str, "Rejecting DID registration in pool: Identity already registered (replay prevention)");
                    return TransactionValidationOutcome::Invalid(
                        transaction,
                        InvalidTransactionError::TxTypeNotSupported.into(),
                    );
            ...
            if let Ok(reg) = registry_lock.read() {
                ...
                let is_claim = if let Some(to) = transaction.to() {
                    to == crate::system_registry::SYSTEM_RECEIVE_HOOK
                } else {
                    false
                };
                ...
                if !has_did {
                    if is_claim {
                        // Receive / claim blocks must have more than the gas needed to move them
                        let min_gas_needed = alloy_primitives::U256::from(21000u64);
                        let tx_val = transaction.value();
                        if tx_val < min_gas_needed {
                            tracing::warn!(?sender, ?tx_val, "Rejecting claim in pool: Claim amount must exceed minimum gas needed to move funds (21000 wei)");
                            return TransactionValidationOutcome::Invalid(
                                transaction,
                                InvalidTransactionError::TxTypeNotSupported.into(),
                            );
        ...
        let is_self_claim = if let Some(recipient) = transaction.to() {
            recipient == sender && transaction.input().is_empty()
        } else {
            false
        };

        if gas_price == 0 && !is_system_operation && !is_did_reg && !is_self_claim {
            return TransactionValidationOutcome::Invalid(
                transaction,
                InvalidTransactionError::TxTypeNotSupported.into(),
            );
        ...
        if quantum_threat && !is_did_reg {
            let Ok((scheme, pk, sig)) = sovereign_crypto::unpack_pq_envelope(transaction.input().as_ref()) else {
                tracing::warn!(
            ...
            let configured_scheme = sovereign_crypto::parse_scheme(&default_pq_scheme).unwrap_or(sovereign_crypto::SignatureScheme::MlDsa);
            ...
            if sovereign_crypto::verify_signature(scheme, &pk, msg, &sig, true).is_err() {
                return TransactionValidationOutcome::Invalid(
                    transaction,
                    InvalidTransactionError::TxTypeNotSupported.into(),
                );
        ...
        let final_outcome = match inner_outcome {
            TransactionValidationOutcome::Invalid(tx, err) => {
                ...
                if is_did_reg || is_sys_target {
                    ...
                    let initial_balance = self.client.get_balance(sender);
                    let lattice_balance = if let Ok(reg) = registry_lock.read() {
                        compute_lattice_balance(sender, &reg, initial_balance)
                    } else {
                        initial_balance
                    };
                    ...
                    if gas_price == 0 || lattice_balance >= needed {
                        ...
                        tracing::warn!(?sender, ?lattice_balance, ?needed, "Rejecting system transaction: Insufficient genuine balance");
                        TransactionValidationOutcome::Invalid(tx, err)
                    }
                } else if let Ok(reg) = registry_lock.read() {
                    let state_nonce = self.client.get_nonce(sender);
                    if state_nonce != transaction.nonce() {
                        tracing::warn!(?sender, ?state_nonce, tx_nonce = transaction.nonce(), "Rejecting invalid nonce for transaction");
                        return TransactionValidationOutcome::Invalid(tx, err);
                    }
                    let initial_balance = self.client.get_balance(sender);
                    let lattice_balance = compute_lattice_balance(sender, &reg, initial_balance);
                    let gas_price = tx.gas_price().unwrap_or_else(|| tx.max_fee_per_gas());
                    ...
                    if lattice_balance >= needed {
                        ...
                        tracing::warn!(?sender, ?lattice_balance, ?needed, "Lattice balance insufficient in fallback");
                        TransactionValidationOutcome::Invalid(tx, err)
                    }
                } else {
                    tracing::warn!(?sender, "Could not acquire registry lock in fallback");
                    TransactionValidationOutcome::Invalid(tx, err)
                }
            }
            other => other,
    ...
    Evm: ConfigureEvm<Primitives = PrimitivesTy<Types>> + Clone + 'static,
{
    ...
    async fn build_pool(
        ...
        let mut config = ctx.pool_config();
        config.minimal_protocol_basefee = self.pool_config.minimal_protocol_basefee;
        config.minimum_priority_fee = self.pool_config.minimum_priority_fee;

        let transaction_pool =
            Pool::new(validator, FCFSOrdering::new(), blob_store, config);
...
pub fn validate_witness<T: PoolTransaction>(
    ...
    if let Ok((scheme, pk, sig)) = sovereign_crypto::unpack_pq_envelope(&transaction.witness) {
        ...
        if sovereign_crypto::verify_signature(scheme, &pk, msg, &sig, quantum_threat).is_err() {
            return false;
        }
...
mod tests {
    ...
    fn test_witness_validation() {
        use sovereign_crypto::{pack_pq_envelope, SignatureScheme};
        ...
        let derived_addr = alloy_primitives::Address::from(sovereign_crypto::derive_address(sovereign_crypto::HashScheme::Poseidon, &pk_bytes));
