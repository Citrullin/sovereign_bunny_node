...
use std::sync::Arc;
use crate::dataplane::DataPlaneDriver;
...
impl BgpRouter {
    ...
    #[must_use]
    pub fn new() -> Self {
        let hw_driver = Arc::new(crate::dataplane::MockHardwareDriver::new());
    ...
    pub fn receive_based_mesh_packet(
        ...
        wg_packet: &[u8],
        out_buf: &mut [u8],
    ) -> Result<Option<crate::relay_mesh::BasedMeshWrapper>, String> {
        ...
        let packet = if decapsulated.len() == 131_072 {
            crate::relay_mesh::BasedMeshWrapper::from_eip4844_blob_bytes(&decapsulated)
                .map_err(|e| format!("Failed to decode EIP-4844 blob packet: {e}"))?
        } else {
            crate::relay_mesh::BasedMeshWrapper::from_bytes(&decapsulated)
...
mod tests {
    ...
    fn test_hardware_offload_integration() {
        let mock_driver = Arc::new(crate::dataplane::MockHardwareDriver::new());
    ...
    fn test_pied_piper_7_hop_bandwidth_allocation() {
        ...
        use crate::relay_mesh::{BasedMeshWrapper, ProofScheme};
