//! Epoch-based deterministic subset election module for cross-manifold relaying.
//!
//! Randomly samples validators from the routing pool per epoch using VRF-style deterministic sorting.

use alloy_primitives::Address;
use std::collections::HashSet;
use alloy_primitives::keccak256;

/// Represents an Epoch deterministic subset election state.
pub struct EpochSubsetElection {
...
pub type SnowSubsetElection = EpochSubsetElection;

impl EpochSubsetElection {
    /// Creates a new `EpochSubsetElection` helper.
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn test_epoch_subset_election_success() {
        let mut election = EpochSubsetElection::new(42);
        ...
        let mut election2 = EpochSubsetElection::new(42);
        let res2 = election2.trigger_election(&pool, 100, 2);
        assert!(res2.is_ok());
        assert_eq!(election.current_subset, election2.current_subset);

        // Test epoch rotation: different epoch should yield a potentially different sort/subset
        let mut election3 = EpochSubsetElection::new(42);
        let _ = election3.trigger_election(&pool, 200, 2);
        assert_eq!(election3.current_subset.len(), 2);
    }

    #[test]
    fn test_epoch_subset_election_empty_pool() {
        let mut election = EpochSubsetElection::new(42);
    ...
    #[test]
    fn test_epoch_subset_election_clamped_sample_size() {
        let mut election = EpochSubsetElection::new(42);
        let mut pool = HashSet::new();
        let addr1 = Address::repeat_byte(0x01);
        pool.insert(addr1);
